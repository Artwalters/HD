...
map.on('load', async () => {
    // Remove any conflicting 3D buildings layers (already in custom style)

    // Load business data first
    await loadBusinessData();
    
    if (businessesGeoJSON) {
        // Add business markers using native Mapbox layers
        addBusinessMarkers();
    } else {
...
// Function to add business markers using native Mapbox layers
function addBusinessMarkers() {
    console.log('Adding business markers...', businessesGeoJSON);
    
    if (!businessesGeoJSON || !businessesGeoJSON.features) {
    ...
    if (map.getLayer('business-labels')) {
        map.removeLayer('business-labels');
    }
    if (map.getLayer('business-markers')) {
        map.removeLayer('business-markers');
    }
    ...
    console.log('Added source with', businessesGeoJSON.features.length, 'features');
    ...
    map.addLayer({
        ...
        paint: {
            'circle-radius': [
                ...
                ['linear'],
                ['zoom'],
                CONFIG.MARKER_ZOOM.min, 6,
                CONFIG.MARKER_ZOOM.small, 10,
                CONFIG.MARKER_ZOOM.medium, 14,
                CONFIG.MARKER_ZOOM.large, 18
            ],
    ...
    console.log('Added circle markers layer');

    // Add text labels layer with simple text instead of emojis
    map.addLayer({
        id: 'business-labels',
        type: 'symbol',
        source: 'businesses',
        layout: {
            'text-field': [
                'case',
                ['==', ['get', 'category'], 'Horeca'], '•',
                ['==', ['get', 'category'], 'Restaurant'], 'R',
                ['==', ['get', 'category'], 'Retail'], 'S',
                ['==', ['get', 'category'], 'Mode'], 'M',
                ['==', ['get', 'category'], 'Dienstverlening'], 'D',
                ['==', ['get', 'category'], 'Bakkerij'], 'B',
                ['==', ['get', 'category'], 'Cultuur'], 'C',
                '•'
            ],
            'text-size': [
                'interpolate',
                ...
                CONFIG.MARKER_ZOOM.small, 12,
                CONFIG.MARKER_ZOOM.medium, 16,
                CONFIG.MARKER_ZOOM.large, 20
            ],
            'text-anchor': 'center',
    ...
    console.log('Added text labels layer');

    // Add click event for advanced popups
    map.on('click', 'business-markers', async (e) => {
        const coordinates = e.features[0].geometry.coordinates.slice();
        const properties = e.features[0].properties;

        console.log('Clicked on:', properties.name);
        ...
        // Handle existing popup using closeActivePopup function
        if (window.activePopup) {
            closeActivePopup();
            await new Promise(resolve => setTimeout(resolve, 400));
        }

        // Create new popup with advanced styling
    ...
    map.on('mouseenter', 'business-markers', () => {
        map.getCanvas().style.cursor = 'pointer';
    });

    map.on('mouseleave', 'business-markers', () => {
        map.getCanvas().style.cursor = '';
    });

    console.log('Business markers setup complete');
    
    // Add event listeners for auto-closing popups on map navigation
    map.on('dragstart', closeActivePopup);
    map.on('zoomstart', closeActivePopup);
    map.on('movestart', closeActivePopup);
    map.on('pitchstart', closeActivePopup);
    map.on('rotatestart', closeActivePopup);
    
    // Close popup when clicking elsewhere on map (not on markers)
    map.on('click', (e) => {
        const features = map.queryRenderedFeatures(e.point, { layers: ['business-markers'] });
...
// Function to fly to business location
function flyToBusiness(lng, lat) {
    map.flyTo({
        center: [lng, lat],
        zoom: 17,
        essential: true
...
// Function to close active popup with animation
function closeActivePopup() {
    if (window.activePopup) {
        const popupContent = window.activePopup.getElement().querySelector(".mapboxgl-popup-content");
        if (popupContent) {
...
function createPopupContent(properties) {
    ...
    // Create HTML content - simplified for business listings
    const html = `
...
// Function to setup popup interactions
